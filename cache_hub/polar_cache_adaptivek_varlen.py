...
Adaptive top-k/value-weighted:
   ...
   - **每一步都重算 adaptive_k**
        ...
        每步真实 K 上界:
            effective_max_k = min(
                _adaptive_topk_buffer_cap,                    # init 时按 max_length 推导的硬上界
                cand_len,                                     # kernel 不允许 K_out > C
                max(ceil(cand_len * max_k_ratio), min_k),     # 动态比例 + min_k 兜底
            )
...
# 默认码本路径：相对于本文件解析（cache_hub/../turboquant/codebooks/...），
# 避免硬编码 /data/qwang/... 在新机器上失效。
# 优先级：环境变量 ANNCACHE_CODEBOOK_PATH > 模块相对路径
_DEFAULT_CODEBOOK_PATH = os.environ.get(
    "ANNCACHE_CODEBOOK_PATH",
    str(
...
# Varlen path: 用 flash_attn_varlen_func 实现 head-wise 自适应 K
# 每个 (batch, kv_head) 当一条独立序列，cu_seqlens_k 编码 per-head 不同长度
try:
...
class polar_cache(KV_Cache):
    ...
    def __init__(
        ...
        num_gpus: int,
        model_size: int,
        codebook_path: str = _DEFAULT_CODEBOOK_PATH,
        ...
        adaptive_topk_threshold: float = 0.85,  # 累积 attention mass 阈值
        adaptive_topk_min_k: int = 50,  # per-head 最小 k(0=不启用 min_k 兜底)
        adaptive_topk_max_k_ratio: float = 0.1,  # per-head 最大 k = ceil(cand_len * ratio)，buffer 上界自动从 max_length 推导
    ) -> None:
        ...
        # NOTE: 这一版只支持 offload 模式（GPU 只放 sink+local+update_buffer，
        # retrieval zone 全部 offload 到 CPU pinned）。非 offload 路径已删除。
        assert enable_offload, (
            "polar_cache_value_adaptive_k_cuda 只支持 enable_offload=True；"
            "若需要全 GPU 非 offload 路径，请改用 polar_cache.py 等老版本"
        )
        self.enable_offload = True
        # 自适应 Top-k 参数
        self.adaptive_topk_threshold = adaptive_topk_threshold
        # min_k 兜底：当 attention-mass 触发的 k 过小时，把每个 head 的 k 抬到至少 min_k；
        self.adaptive_topk_min_k = int(adaptive_topk_min_k) if adaptive_topk_min_k is not None else 0
        if self.adaptive_topk_min_k < 0:
            self.adaptive_topk_min_k = 0
        # max_k = ceil(cand_len * ratio)，每步动态。0/None 表示禁用 ratio（max_k 退化到 cand_len 自身）。
        self.adaptive_topk_max_k_ratio = (
            float(adaptive_topk_max_k_ratio) if adaptive_topk_max_k_ratio is not None else 0.0
        )
        if self.adaptive_topk_max_k_ratio < 0.0:
            self.adaptive_topk_max_k_ratio = 0.0
        if self.adaptive_topk_max_k_ratio > 1.0:
            print(
                f"[Init] WARN adaptive_topk_max_k_ratio({self.adaptive_topk_max_k_ratio}) > 1.0，clamp 到 1.0",
                flush=True,
            )
            self.adaptive_topk_max_k_ratio = 1.0
        # ===== Varlen 路径：head-wise 自适应 K，max_k=800/min_k=50 写死 =====
        # 每个 (batch, kv_head) 单独决定自己的 K（按 benefit 阈值），范围 [50, 800]。
        # 下游 attention 走 flash_attn_varlen_func，把 (bs, kv_heads) 当 virtual batch；
        # buffer_cap 直接固定为 800（adaptive_topk2.cu 的 K_out / topk_indices 第三维）。
        self._HARDCODED_MAX_K = int(os.environ.get("ADAPTIVE_TOPK_MAX_K", "800"))
        self._HARDCODED_MIN_K = int(os.environ.get("ADAPTIVE_TOPK_MIN_K", "100"))
        self._adaptive_topk_buffer_cap = self._HARDCODED_MAX_K
        print(
            f"[Init] adaptive_topk (varlen): threshold={self.adaptive_topk_threshold} "
            f"hardcoded min_k={self._HARDCODED_MIN_K} max_k={self._HARDCODED_MAX_K} "
            f"→ buffer_cap={self._adaptive_topk_buffer_cap}",
            flush=True,
        )
        try:
            with open(codebook_path, 'r') as f:
        ...
        if self.enable_offload:
            ...
            # Stage 2 的 packed varlen 路径直接把 H2D 结果写进最终 flash_attn_varlen 的 [total_k, dim] 输入 buffer（每步按真实 sum(adaptive_k) 动态 alloc），
            # 不再需要 [bs, H, max_topk, dim] 的中转 dst buffer，省 PCIe 传输又省显存。
            self._topk_keys_buffer = None
            self._topk_values_buffer = None

            # 预分配 adaptive_topk kernel 的 adaptive_k output buffer（int64, 仅 [bs, H]）
            # topk_indices output 改为 per-call alloc：max_k_ratio 模式下 topk_cap 每步动态，
            # kernel 用 `bh * K_out` 寻址要求 stride(1)==K_out，固定容量 buffer 切片会让槽位错乱。
            self._adaptive_topk_k_buffer = torch.empty(
                (batch_size, self.kv_head),
                dtype=torch.int64, device=self.device,
            ).contiguous()

            # ===== Varlen attention 持久 buffer =====
            # decode 每步 q 长度恒为 1，cu_seqlens_q = [0, 1, 2, ..., V]，永不变 → 一次建好复用
            # cu_seqlens_k_cpu 是每步 cumsum 的 CPU 暂存 buffer，复用避免每步 alloc
            assert _flash_attn_varlen_func is not None, (
                "flash_attn_varlen_func 不可用，请安装 flash-attn 才能用 varlen 路径"
            )
            _V = self.batch_groups  # bs * kv_head
            self._varlen_cu_seqlens_q = torch.arange(
                0, _V + 1, dtype=torch.int32, device=self.device,
            )
            # 复用避免每步 alloc/zeros：
            #   _varlen_cu_seqlens_k_cpu  : (sl_end + adaptive_k) prefix sum，决定 packed
            #                                K/V 在 [total_k, dim] 中每个 (b, h) 段的起点
            #   _varlen_cu_seqlens_topk_cpu: adaptive_k prefix sum，决定 packed indices 在
            #                                [total_topk] 中每个 (b, h) 段的起点
            self._varlen_cu_seqlens_k_cpu = torch.zeros(_V + 1, dtype=torch.int32)
            self._varlen_cu_seqlens_topk_cpu = torch.zeros(_V + 1, dtype=torch.int32)
            ...
            self._offload_keys_buffer = [
                torch.empty((batch_size, self.kv_head, self.dynamic_update_interval, head_dim),dtype=dtype, device=self.device)
        ...
        # Varlen 路径下每层 K 不再是单值，存整张 [bs, kv_heads] tensor 引用，最后一层一次 d2h
        self._adaptive_k_log_max = [None] * layer_num             # 兼容旧字段：max(adaptive_k) 标量
        self._adaptive_k_log_full = [None] * layer_num            # 新增：[bs, kv_heads] tensor，per-head 维度

        # ===== 可选：per-(step, layer, head) adaptive_k 全轨迹 dump =====
        # 启用方式: 环境变量 POLAR_ADAPTIVE_TOPK_DUMP_PATH=/path/to/foo.npz
        # 写出: np.savez_compressed(..., step=[N], adaptive_k=[N, L, bs, kv_heads])
        #       N=触发 retrieval 的 decode 步数，dtype int32
        # 用途: 离线分析步间 K 稳定性 → 决定是否能每 R 步刷一次 adaptive_k
        # 开销: 仅在 env var 设置时才存；每步追加 1 个 [L, bs, kv_heads] CPU numpy
        #       (Qwen3-4B: 36*1*8 = 288 int32 = 1.1KB/step，几千步 ~3MB host 内存)
        self._adaptive_k_dump_path = os.environ.get("POLAR_ADAPTIVE_TOPK_DUMP_PATH", "")
        self._adaptive_k_dump_history: list[tuple[int, np.ndarray]] = []
        if self._adaptive_k_dump_path:
            atexit.register(self._dump_adaptive_k_history)
            print(
                f"[Init] adaptive_k history dump ENABLED → {self._adaptive_k_dump_path}",
                flush=True,
            )
        self.context = 0
        ...
        try:
            from .gather_trans.trans_h2d import h2d_gather_kv, h2d_gather_kv_packed
            self._h2d_gather_kv = h2d_gather_kv
            self._h2d_gather_kv_packed = h2d_gather_kv_packed
        ...
        # 预加载 Adaptive Top-k CUDA extension（v2: 融合 cand_scores cast+scale + cand_vnorm gather +
        # benefit + radix_select + sort + adaptive_k + cand_idx gather）
        # 失败时自动回退到 torch 版本，不影响主流程
        try:
            from .adaptive_topk.adaptive_topk import load_kernel_module as _load_adaptive_topk_module
            self._adaptive_topk_kernel = _load_adaptive_topk_module(
                "adaptive_topk2.cu", "adaptive_topk2",
                cuda_flags=("-O3", "-std=c++17", "--use_fast_math"),
            )
            print(f"[Init]   adaptive_topk loaded ({_time.time()-_t0:.1f}s)", flush=True)
        except Exception as e:
            print(f"[Init]   adaptive_topk FAILED: {e} (fallback to torch impl)", file=sys.stderr, flush=True)
            traceback.print_exc(file=sys.stderr)
            sys.stderr.flush()
            self._adaptive_topk_kernel = None

        # 是否启用 CUDA 版 adaptive_topk（kernel 加载失败时强制为 False）
        # 通过 env POLAR_USE_CUDA_ADAPTIVE_TOPK=0 可在运行时禁用便于 A/B
        _env_use_cuda_atk = os.environ.get("POLAR_USE_CUDA_ADAPTIVE_TOPK", "1")
        self._use_cuda_adaptive_topk = (
            self._adaptive_topk_kernel is not None and _env_use_cuda_atk not in ("0", "false", "False")
        )
        # adaptive_k / topk_indices 输出 buffer 改为 per-call torch.empty（PyTorch caching
        # allocator 自动复用），避免预分配 buffer 在 topk_cap < max_k 时切片非 contiguous
        # 导致 CUDA kernel 写越界。
        print(f"[Init]   use_cuda_adaptive_topk = {self._use_cuda_adaptive_topk}", flush=True)
    ...
    def _update_polar_index(self):
        ...
        for ldx in range(self.layer_num):
            # 获取需要编码的新增部分（即将 offload 到 CPU 的那段）
            # GPU 布局: [sink | local | update_buffer], 需要编码的是 [sink : sink + num_new_tokens]（旧 Local + Buffer 前半）
            ...
            # _sl_vnorm_cache 容量 = gpu_kv_capacity = sink+local+update_buffer，
            # 即将 offload 的那段是 [sink_size : sink_size + num_new_tokens]
            ...
            # 1. 把 Local 最老的 num_new_tokens 个 tokens offload 到 CPU, 这些 tokens 从 GPU local 区域移出，进入 CPU retrieval zone
    ...
    def decode_update_kv_cache(self, key_states, value_states, layer_idx):
        ...
        # GPU 布局: [Sink][Local][UpdateBuffer]
    ...
    def compute(self, queries, layer_idx):
        ...
        # Offload 模式下 cpu_valid_end 就是实际限制：cpu_valid_end <= encoded_len <= retrieval_len
        valid_length = self.cpu_valid_end[layer_idx]
        ...
        if use_full_attention:
            ...
            # NVTX 范围覆盖 sink+local 输入准备 + adaptive topk 计算两段：
            # 这样 timeline 上 collision_based_topk_batch / compute_adaptive_topk /
            # h2d_gather_kv / compute_attention 四个 outer block 边界对齐，方便对比
            nvtx.range_push("adaptive_k0_pre")
            # GPU 布局: [Sink][Local][UpdateBuffer]，sink+local 直接前缀切片即可
            sink_local_end = self.sink_size + local_len
            sink_local_keys = self.unified_keys_gpu[layer_idx][:, :, :sink_local_end, :]
            sink_local_values = self.unified_values_gpu[layer_idx][:, :, :sink_local_end, :]
            ...
            adaptive_k, indices_packed, global_k = self._compute_adaptive_topk(
            ...
            # indices_packed: [total_topk] int64 GPU, (b, h) row-major concat,
            # 第 vh 段长度 = adaptive_k[b, h]，配 cu_seqlens_topk 切。

            # 收集 per-head adaptive_k：直接存 GPU tensor 引用，最后一层一次性 d2h，避免每层都 sync。
            # global_k 已经是 int（kernel 里 .item() 算的），同步一次 + tensor stack 一次 = 1 sync/step。
            self._adaptive_k_log_max[layer_idx] = global_k
            # clone：_adaptive_topk_k_buffer 是跨层共享 buffer，不 clone 的话 36 层的 view
            # 全指向同一块 GPU 内存，最后 stack 出来读到的全是 L35 那一层的值。
            # 拷贝量 = bs * kv_heads * 8B（int64），≪ 1KB/layer，完全可以忽略。
            self._adaptive_k_log_full[layer_idx] = adaptive_k.detach().clone()

            # [Profiling] per-step adaptive_k 统计打印；nsys 跑 timing 时关掉避免引入抖动
            if layer_idx == self.layer_num - 1:
                # 一次 d2h 把所有层的 [bs, kv_heads] 拉下来，shape [L, bs, kv_heads]
                _full = torch.stack(self._adaptive_k_log_full, dim=0).cpu()  # int64

                # 可选：把整张 [L, bs, kv_heads] 追加到 history buffer，atexit 时落 npz
                # （已经在 CPU，不会再额外触发 sync）
                if self._adaptive_k_dump_path:
                    self._adaptive_k_dump_history.append(
                        (int(total_length), _full.numpy().astype(np.int32, copy=True))
                    )

                _per_layer_max = _full.amax(dim=(1, 2)).tolist()             # [L]
                _per_layer_min = _full.amin(dim=(1, 2)).tolist()
                _per_layer_avg = _full.float().mean(dim=(1, 2)).tolist()

                _max_str = " ".join(f"{v:>4d}" for v in _per_layer_max)
                _min_str = " ".join(f"{v:>4d}" for v in _per_layer_min)
                _avg_str = " ".join(f"{v:>4.0f}" for v in _per_layer_avg)
                print(
                    f"[AdaptiveK-VW] step={total_length} L0..L{self.layer_num-1}\n"
                    f"  per_layer_max: {_max_str}\n"
                    f"  per_layer_min: {_min_str}\n"
                    f"  per_layer_avg: {_avg_str}",
                    flush=True,
                )

                # POLAR_ADAPTIVE_TOPK_PRINT_K=1：每层每个 head 全 dump（很吵，仅调试）
                _atk_dbg = os.environ.get("POLAR_ADAPTIVE_TOPK_PRINT_K", "")
                if _atk_dbg and _atk_dbg != "0":
                    _full_list = _full.tolist()  # [L][bs][kv_heads]
                    for _li, _bs_rows in enumerate(_full_list):
                        for _b, _row in enumerate(_bs_rows):
                            _per_head = " ".join(f"h{_h}={_k}" for _h, _k in enumerate(_row))
                            print(f"  [AdaptiveK-VW] L{_li} b{_b}: {_per_head}", flush=True)

                self._adaptive_k_log_max = [None] * self.layer_num
                self._adaptive_k_log_full = [None] * self.layer_num
            del all_cand_scores, all_cand_indices

            # ============ Stage 2 packed varlen pipeline ============
            # 一次性把 H2D gather + packing 都按 head-wise adaptive_k 真值做：
            #   - H2D: 只搬 sum(adaptive_k) 行，省下 ~46% PCIe 带宽
            #   - dst 直接是 flash_attn_varlen_func 的最终 packed buffer，省一次显存复制
            #     和 ~3MB 中转 buffer
            # 关键变量含义：
            #   indices_packed   [total_topk]   GPU int64  (b,h) row-major concat 的 KV indices
            #   adaptive_k       [bs, kv_heads] GPU int64  per-head 实际 K
            #   global_k         int            host       max(adaptive_k)，作 H2D kernel 的
            #                                              grid Y 上界（kernel 内每段 slot
            #                                              超过自己 adaptive_k 时早退）
            #   k_packed         [total_k, 1, hd] GPU bf16  最终 flash 输入；每段
            #                    [cu_seqlens_k[vh] .. cu_seqlens_k[vh]+sl_end) 是 sink+local，
            #                    [+sl_end .. cu_seqlens_k[vh+1]) 是 head-wise topk
            kv_h = self.kv_head
            grp = self.group_size
            hd = self.head_dim
            V = self.batch_groups  # = bs * kv_h，init 时锁定

            nvtx.range_push("compute_attention_varlen")

            # ---- 1) 单次 d2h sync：从 adaptive_k 推导出所有 host 端需要的标量/前缀和 ----
            # 这一处 .cpu() 不可避免：flash_attn_varlen_func 要 max_seqlen_k 是 Python int，
            # 同时 caller 也需要 total_k 来 alloc packed buffer、需要 cu_seqlens_*_cpu 做 D2H copy。
            ak_cpu = adaptive_k.detach().to('cpu', dtype=torch.int32)            # [bs, kv_heads]
            ak_flat_cpu = ak_cpu.reshape(-1)                                     # [V]
            seqlens_k_cpu = ak_flat_cpu + sink_local_end                         # [V]
            cu_seqlens_k_cpu = self._varlen_cu_seqlens_k_cpu
            torch.cumsum(seqlens_k_cpu, dim=0, out=cu_seqlens_k_cpu[1:])
            cu_seqlens_topk_cpu = self._varlen_cu_seqlens_topk_cpu
            torch.cumsum(ak_flat_cpu, dim=0, out=cu_seqlens_topk_cpu[1:])
            total_k = int(cu_seqlens_k_cpu[-1].item())
            max_seqlen_k = int(seqlens_k_cpu.max().item())

            cu_seqlens_k = cu_seqlens_k_cpu.to(self.device, non_blocking=True)
            cu_seqlens_topk = cu_seqlens_topk_cpu.to(self.device, non_blocking=True)

            # ---- 2) 直接 alloc 最终 packed buffer，flash_attn_varlen_func 期望 [total, h, d] ----
            # num_heads=1：每个 (b, h) 已经独立成一条 virtual batch，共享 1 个 kv head。
            k_packed = torch.empty(total_k, 1, hd, dtype=self.dtype, device=self.device)
            v_packed = torch.empty(total_k, 1, hd, dtype=self.dtype, device=self.device)
            k_packed_2d = k_packed.view(total_k, hd)   # alias for slice-assign / kernel write
            v_packed_2d = v_packed.view(total_k, hd)

            # ---- 3) sink+local 段：GPU→GPU 的 V 次小切片 copy，写到每段前 sl_end 行 ----
            # sink_local_keys 是 unified_keys_gpu 的带步长视图（slice dim=2 -> 非 contig），
            # flatten(0, 1) 走 stride view 零拷贝；每个 vh 拿到 [sl_end, hd] 子视图。
            nvtx.range_push("pack_sink_local")
            sl_v_k = sink_local_keys.flatten(0, 1)     # [V, sl_end, hd] strided view
            sl_v_v = sink_local_values.flatten(0, 1)
            cu_list = cu_seqlens_k_cpu.tolist()        # host int list, V+1 项
            for vh in range(V):
                start = cu_list[vh]
                k_packed_2d[start:start + sink_local_end] = sl_v_k[vh]
                v_packed_2d[start:start + sink_local_end] = sl_v_v[vh]
            nvtx.range_pop()

            # ---- 4) topk 段：H2D kernel 直接写到 k_packed/v_packed，跳过中转 buffer ----
            # 每个 (b, h) 段从 cu_seqlens_k[vh] + sl_end 起，连续写 adaptive_k[b, h] 行。
            # 总 PCIe 字节数 = sum(adaptive_k) * hd * 2B/elem * 2 (K+V)，按真实 K 计费。
            nvtx.range_push("h2d_gather_kv_packed")
            self._h2d_gather_kv_packed(
            ...
            # ---- 5) Q reshape: [bs, 1, num_q_heads, head_dim] → [V, group_size, head_dim] ----
            # GQA: q-head [h*grp : (h+1)*grp] 共享 kv-head h
            q_packed = (queries.view(bs, 1, kv_h, grp, hd).permute(0, 2, 1, 3, 4)
                       .reshape(V, grp, hd).contiguous())
            ...
            out_packed = _flash_attn_varlen_func(
                ...
                k=k_packed,
                v=v_packed,
                cu_seqlens_q=self._varlen_cu_seqlens_q,
                cu_seqlens_k=cu_seqlens_k,
                max_seqlen_q=1,
                max_seqlen_k=max_seqlen_k,
                causal=False,
            )  # [V, group_size, head_dim]

            # ---- 7) 还原回 [bs, 1, num_q_heads, head_dim] ----
            attn_out = (
                out_packed.view(bs, kv_h, 1, grp, hd)
                          .permute(0, 2, 1, 3, 4)
                          .reshape(bs, 1, kv_h * grp, hd)
                          .contiguous()
            )
            nvtx.range_pop()
            del sink_local_keys, sink_local_values
            del k_packed, v_packed, q_packed, out_packed
            del indices_packed
    ...
    def _compute_adaptive_topk(
        ...
        Returns:
            ...
            indices_packed:[total_topk]     int64 GPU contiguous，concat 顺序按 (b, h)
                                            row-major；vh = b*kv_heads + h 对应的段长度
                                            == adaptive_k[b, h]。配合 cu_seqlens_topk
                                            （由 caller 用 adaptive_k cumsum 得到）即可
                                            做 head-wise 变长查找。
            global_k:      int              max(adaptive_k)，用作 H2D kernel 的 grid Y dim
                                            上界（每个 (b,h) 段超过 adaptive_k[b,h] 的
                                            slot 在 kernel 内早退）。
        """
        # v2 NVTX subrange 简化为 4 段（vnorm_gather + kernel_prep 都已融合进 kernel）：
        #   adaptive_k1_qk_logits / adaptive_k2_cuda_kernel
        #   adaptive_k3_max_k / adaptive_k3_topk_slice
        cand_len = all_cand_scores.shape[-1]
        sl_len = sink_local_keys.shape[2]
        bs, kv_heads = query_avg.shape[:2]
        # ===== Varlen 路径：写死 max_k=800, min_k=50 =====
        # 仅当 cand_len < max_k 时被迫降到 cand_len（kernel 要求 cand_len >= K_out）。
        effective_max_k = min(self._HARDCODED_MAX_K, cand_len)
        effective_min_k = min(self._HARDCODED_MIN_K, effective_max_k)
        topk_cap = effective_max_k

        # 1. QK logits（matmul + scale 仍走 PyTorch / cuBLAS，tensor core）
        # cand_logits 不再预算，cast+scale 已融合进 kernel（直接读 bf16 cand_scores）
        nvtx.range_push("adaptive_k1_qk_logits")
        logits_sl = torch.einsum('bhd,bhkd->bhk', query_avg, sink_local_keys).to(torch.float32) * self._attn_scale
        nvtx.range_pop()

        # 2a. CUDA 路径（v2 fused kernel）：吞掉 cand cast+scale、cand_vnorm gather、
        #     sl_vnorm contiguous（用 stride 寻址），与原 step 4 的 benefit→radix→sort→adaptive_k 一起跑
        if self._use_cuda_adaptive_topk and self._adaptive_topk_kernel is not None:
            nvtx.range_push("adaptive_k2_cuda_kernel")
            adaptive_k_buf = self._adaptive_topk_k_buffer[:bs]
            # topk_indices_buf 必须每步按 topk_cap 现 alloc：kernel 用 `bh * K_out` 寻址，
            topk_indices_buf = torch.empty((bs, kv_heads, topk_cap), dtype=torch.int64, device=self.device)
            self._adaptive_topk_kernel.adaptive_topk(
                logits_sl,                          # [bs, H, sl]    fp32 contiguous
                all_cand_scores,                    # [bs, H, C]     bf16 contiguous（kernel 内部 cast+scale）
                float(self._attn_scale),
                sl_vnorm,                           # [bs, H, sl]    fp32 stride view（kernel 用 stride(1) 寻址）
                all_cand_indices,                   # [bs, H, C]     int64 contiguous
                self.value_norm_gpu[layer_idx],     # [bs, H, *]     fp32 contiguous（kernel 内部 gather cand_vnorm）
                int(self.sink_size),
                float(threshold),
                int(topk_cap),
                adaptive_k_buf,
            ...
            # 同时做两件事：
            #   (a) min_k 兜底：把 adaptive_k 抬到至少 effective_min_k
            #   (b) max_k 修正：把 adaptive_k 截到 effective_max_k = min(topk_cap, cand_len)
            # kernel 输出的 topk_indices_buf 已按 benefit 降序排好（前 cand_len 个有效），
            # 下游只 slice 到 [:, :, :global_k]，clamp 完保证 global_k <= cand_len 即可。
            adaptive_k_buf.clamp_(min=max(1, effective_min_k), max=max(1, effective_max_k))
            nvtx.range_pop()

            # GPU→CPU 同步点：单独测 d2h sync（max + .item()）的实际等待开销
            nvtx.range_push("adaptive_k3_max_k")
            global_k = int(adaptive_k_buf.max().item())
            nvtx.range_pop()

            # 把 [bs, kv_heads, topk_cap] 的 dense indices 按 head-wise adaptive_k
            # mask 出 packed [total_topk]，给下游 h2d_gather_kv_packed 直接用。
            nvtx.range_push("adaptive_k3_pack_indices")
            indices_packed = self._pack_indices(topk_indices_buf, adaptive_k_buf, topk_cap)
            nvtx.range_pop()
            return adaptive_k_buf, indices_packed, global_k




        # 2b. Torch fallback：保留原始实现（kernel 失败 / A/B debug）
        # fallback 路径不在热路径上，惰性补算 logits_cand / cand_vnorm 即可
        logits_cand = all_cand_scores.to(torch.float32) * self._attn_scale
        abs_cand_indices = all_cand_indices.long() + self.sink_size
        cand_vnorm = torch.gather(self.value_norm_gpu[layer_idx], dim=-1, index=abs_cand_indices)
        ...
        # benefit = exp(l - m) * value_norm（未归一化质量；判定式中 1/Z 公因子可约掉）
        benefit = joint_exp * joint_vnorm
        sl_benefit = benefit[:, :, :sl_len].sum(dim=-1, keepdim=True)  # [bs, kv_heads, 1]
        cand_benefit = benefit[:, :, sl_len:]                           # [bs, kv_heads, cand_len]
        # k 不能超过 cand_len（torch.topk 强校验 k <= dim_size，否则 RuntimeError）
        cand_benefit_topk, sort_perm = torch.topk(cand_benefit, k=effective_max_k, dim=-1, largest=True, sorted=True)
        cumsum_benefit = torch.cumsum(cand_benefit_topk, dim=-1)
        # total benefit = sl + 全部 cand；用 cand_benefit.sum 而不是 cumsum_benefit[..., -1:]是因为 topk 之外的尾部也要参与分母（否则 threshold 永远在 cap 处达成）
        ...
        adaptive_k[never_reached] = effective_max_k
        bad_total = (total_benefit_all.squeeze(-1) <= 0)
        if bad_total.any():
            adaptive_k = torch.where(bad_total, torch.ones_like(adaptive_k), adaptive_k)
        # min_k 兜底 + max_k 双向 clamp（理论上 reached 路径已 <= effective_max_k，这里防御性）
        adaptive_k = adaptive_k.clamp(
            min=max(1, effective_min_k),
            max=max(1, effective_max_k),
        )
        global_k = int(adaptive_k.max().item())
        # 用 effective_max_k(=topk_cap) 维度的 dense indices + mask packing，与 CUDA 路径同构
        topk_indices_dense = torch.gather(all_cand_indices, dim=2, index=sort_perm)
        indices_packed = self._pack_indices(topk_indices_dense, adaptive_k, effective_max_k)
        return adaptive_k, indices_packed, global_k
    ...
    def _pack_indices(
        self,
        dense_indices: torch.Tensor,   # [bs, kv_heads, topk_cap]   int64
        adaptive_k: torch.Tensor,      # [bs, kv_heads]             int64
        topk_cap: int,
    ) -> torch.Tensor:
        """把 dense [bs, kv_heads, topk_cap] indices 按 head-wise adaptive_k mask 成
        packed [total_topk] (concat 顺序 = (b, h) row-major)。
        kernel 写出的前 adaptive_k[b,h] 个 slot 已经按 benefit 降序排好，
        因此 mask = `arange(topk_cap) < adaptive_k[b,h]` 直接选前缀即可。
        """
        bs, kv_heads = adaptive_k.shape
        V = bs * kv_heads
        arange_v = torch.arange(topk_cap, device=dense_indices.device)
        mask = arange_v.unsqueeze(0) < adaptive_k.view(V, 1)        # [V, topk_cap]
        return dense_indices.contiguous().view(V, topk_cap)[mask]    # [total_topk]





    def _dump_adaptive_k_history(self) -> None:
        """atexit hook：把 [N_step, L, bs, kv_heads] adaptive_k 轨迹落到 npz。

        启用：env POLAR_ADAPTIVE_TOPK_DUMP_PATH=/path/to/foo.npz
        加载分析：
            d = np.load(path)
            steps, ak = d['step'], d['adaptive_k']     # ak: [N, L, bs, kv_heads] int32
            # 例如某 head 步间 ΔK：np.diff(ak[:, layer, b, h], axis=0)
        """
        if not self._adaptive_k_dump_path or not self._adaptive_k_dump_history:
            return
        try:
            steps = np.array(
                [s for s, _ in self._adaptive_k_dump_history], dtype=np.int32
            )
            arrs = np.stack(
                [a for _, a in self._adaptive_k_dump_history], axis=0
            )  # [N, L, bs, kv_heads] int32
            np.savez_compressed(
                self._adaptive_k_dump_path,
                step=steps,
                adaptive_k=arrs,
            )
            print(
                f"[Dump] adaptive_k history → {self._adaptive_k_dump_path} "
                f"shape={arrs.shape} dtype={arrs.dtype} "
                f"({arrs.nbytes / 1024:.1f} KB raw)",
                flush=True,
            )
        except Exception as _e:
            print(
                f"[Dump] adaptive_k history FAILED: {_e}",
                file=sys.stderr,
                flush=True,
            )
    ...
    def get_candidate_cache_with_rabitq_rerank(
        ...
        # adaptive 模式：下游 adaptive_topk 会按 benefit (m_i = exp(l_i)·||v_i||) 重新排序并选 k，这里按 approx_scores 的全排序
